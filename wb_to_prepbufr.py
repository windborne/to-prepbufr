...
def main():
    ...
    parser.add_argument('-d', '--output_dir',
                        help="If selected, all files will be saved in specified path, only used for netcdf.")
    args = parser.parse_args()

    if (len(args.times) == 1):
        starttime=int(datetime.datetime.strptime(args.times[0], '%Y-%m-%d_%H:%M').
                   replace(tzinfo=datetime.timezone.utc).timestamp())
        endtime=int(datetime.datetime.now(datetime.timezone.utc).timestamp())
    ...
    # For more detailed usage of super_observations function you can refer to:
    # https://windbornesystems.com/docs/api/pip_data#super_observations

    if netcdf_output:
        super_observations(
            start_time=datetime.datetime.fromtimestamp(starttime, tz=datetime.timezone.utc).strftime('%Y-%m-%d_%H:%M'),
            end_time=datetime.datetime.fromtimestamp(endtime, tz=datetime.timezone.utc).strftime('%Y-%m-%d_%H:%M'),
        ...
        if args.combine_missions:
            output_data(observations, 'all', starttime, bucket_hours)
        else:
            by_mission = {}
            for obs in observations:
                if 'mission_name' not in obs:
                    continue
                if obs['mission_name'] not in by_mission:
                    by_mission[obs['mission_name']] = []
                by_mission[obs['mission_name']].append(obs)

            for mission_name, mission_obs in by_mission.items():
                output_data(mission_obs, mission_name, starttime, bucket_hours)
